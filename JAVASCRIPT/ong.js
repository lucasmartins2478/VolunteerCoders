...
function montaTela(problemas) {
  ...
  problemas.forEach((element) => {
    if (
      element.solicitado == true &&
      element.selecionado == true &&
      element.aceito == false
    ) {
...
function aceitarProjeto(problemaId) {
  ...
  fetch(`${problemaApiUrl}/${problemaId}`)
    ...
    .catch((error) => {
      ...
      alert(
        "Erro ao buscar os dados do problema. Por favor, tente novamente mais tarde."
      );
    });
}
...
function montaProjetos(projetos) {
  ...
  projetos.forEach((element) => {
    if (element.entregue == true) {
      const newCard = document.createElement("div");
      newCard.classList.add("card");
      const emDesenvolvimentoDiv = element.iniciado
        ? `<div class="em-desenvolvimento">A equipe finalizou o projeto!</div>`
        : `<div class="aindaNao">A equipe ainda não iniciou o desenvolvimento!</div>`;
      ...
      newCard.classList.add("card");
      const emDesenvolvimentoDiv = element.iniciado
        ? `<div class="em-desenvolvimento">Em desenvolvimento pela equipe!</div>`
        : `<div class="aindaNao">A equipe ainda não iniciou o desenvolvimento!</div>`;
...
async function fetchOng(ongId) {
  try {
    const response = await fetch(
      `https://664f28a4fafad45dfae29755.mockapi.io/api/v1/ong/${ongId}`
    );
